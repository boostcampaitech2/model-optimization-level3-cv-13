...
def search_hyperparam(trial: optuna.trial.Trial):
    ...
    epochs = trial.suggest_int("epochs", low=20, high=20, step=20)
...
def search_model(trial: optuna.trial.Trial):
    ...
    if m2 == "Conv":
        ...
        m2_oup = trial.suggest_int("m2/mb_oup", low=16, high=64, step=8)
        m2_t = trial.suggest_int("m2/mb_t", low=1, high=3, step=2)
        m2_stride = trial.suggest_categorical("m2/mb_s", [1, 2]) # m2_stride
        m2_k = trial.suggest_categorical("m2/mb_k", [3, 5])
        m2_args = [m2_oup, m2_t, m2_k, m2_stride]
    elif m2 == "Fire":
        m2_sq = trial.suggest_int("m2/Fi_sq", low=16, high=32, step=8)
        m2_1x1 = trial.suggest_int("m2/1x1", low=32, high=64, step=16)
        m2_3x3 = trial.suggest_int("m2/3x3", low=32, high=64, step=16)
        m2_args = [m2_sq, m2_1x1, m2_3x3]
    if not m2 == "Pass":
        if m2_stride == 2:
            n_stride += 1
    ...
    if m3 == "Conv":
        ...
        m3_oup = trial.suggest_int("m3/mb_oup", low=16, high=128, step=16)
        m3_t = trial.suggest_int("m3/mb_t", low=1, high=6, step=5)
        m3_stride = trial.suggest_categorical("m3/mb_s", [1, 2]) # m2_stride
        m3_k = trial.suggest_categorical("m3/mb_k", [3, 5])
        m3_args = [m3_oup, m3_t, m3_k, m3_stride] 
    elif m3 == "Fire":
        m3_sq = trial.suggest_int("m3/Fi_sq", low=16, high=32, step=8)
        m3_1x1 = trial.suggest_int("m3/1x1", low=32, high=64, step=16)
        m3_3x3 = trial.suggest_int("m3/3x3", low=32, high=64, step=16)
        m3_args = [m3_sq, m3_1x1, m3_3x3]
    if not m3 == "Pass":
        if m3_stride == 2:
            n_stride += 1
            if n_stride >= MAX_NUM_STRIDE:
    ...
    if m4 == "Conv":
        ...
        m4_oup = trial.suggest_int("m4/mb_oup", low=16, high=256, step=16)
        m4_t = trial.suggest_int("m4/mb_t", low=1, high=6, step=5)
        m4_stride = trial.suggest_categorical("m4/mb_s", [1, 2]) # m2_stride
        m4_k = trial.suggest_categorical("m4/mb_k", [3, 5])
        m4_args = [m4_oup, m4_t, m4_k, m4_stride]
    elif m4 == "Fire":
        m4_sq = trial.suggest_int("m4/Fi_sq", low=16, high=32, step=8)
        m4_1x1 = trial.suggest_int("m4/1x1", low=64, high=128, step=16)
        m4_3x3 = trial.suggest_int("m4/3x3", low=64, high=128, step=16)
        m4_args = [m4_sq, m4_1x1, m4_3x3]
    if not m4 == "Pass":
        if m4_stride == 2:
            n_stride += 1
            if n_stride >= MAX_NUM_STRIDE:
    ...
    if m5 == "Conv":
        ...
        m5_oup = trial.suggest_int("m5/mb_oup", low=16, high=256, step=16)
        m5_t = trial.suggest_int("m5/mb_t", low=1, high=6, step=5)
        m5_stride = trial.suggest_categorical("m5/mb_s", [1, 2]) # m2_stride
        m5_k = trial.suggest_categorical("m5/mb_k", [3, 5])
        m5_args = [m5_oup, m5_t, m5_k, m5_stride]
    elif m5 == "Fire":
        m5_sq = trial.suggest_int("m5/Fi_sq", low=32, high=48, step=8)
        m5_1x1 = trial.suggest_int("m5/1x1", low=96, high=192, step=16)
        m5_3x3 = trial.suggest_int("m5/3x3", low=96, high=192, step=16)
        m5_args = [m5_sq, m5_1x1, m5_3x3]
    if not m5 == "Pass":
        if m5_stride == 2:
            n_stride += 1
            if n_stride >= MAX_NUM_STRIDE:
    ...
    if m6 == "Conv":
        ...
        m6_oup = trial.suggest_int("m6/mb_oup", low=16, high=320, step=16)
        m6_t = trial.suggest_int("m6/mb_t", low=1, high=6, step=5)
        m6_stride = trial.suggest_categorical("m6/mb_s", [1, 2]) # m2_stride
        m6_k = trial.suggest_categorical("m6/mb_k", [3, 5])
        m6_args = [m6_oup, m6_t, m6_k, m6_stride]
    elif m6 == "Fire":
        m6_sq = trial.suggest_int("m6/Fi_sq", low=32, high=64, step=8)
        m6_1x1 = trial.suggest_int("m6/1x1", low=128, high=256, step=16)
        m6_3x3 = trial.suggest_int("m6/3x3", low=128, high=256, step=16)
        m6_args = [m6_sq, m6_1x1, m6_3x3]
    if not m6 == "Pass":
        if m6_stride == 2:
            n_stride += 1
            if n_stride >= MAX_NUM_STRIDE:
    ...
    if m7 == "Conv":
        ...
        m7_oup = trial.suggest_int("m7/mb_oup", low=16, high=480, step=16)
        m7_t = trial.suggest_int("m7/mb_t", low=1, high=6, step=5)
        m7_stride = trial.suggest_categorical("m7/mb_s", [1, 2]) # m2_stride
        m7_k = trial.suggest_categorical("m7/mb_k", [3, 5])
        m7_args = [m7_oup, m7_t, m7_k, m7_stride] 
    elif m7 == "Fire":
        m7_sq = trial.suggest_int("m7/Fi_sq", low=48, high=64, step=8)
        m7_1x1 = trial.suggest_int("m7/1x1", low=128, high=256, step=16)
        m7_3x3 = trial.suggest_int("m7/3x3", low=128, high=256, step=16)
        m7_args = [m7_sq, m7_1x1, m7_3x3]
    if not m7 == "Pass":
        if m7_stride == 2:
            n_stride += 1
            if n_stride >= MAX_NUM_STRIDE:
...
def objective(trial: optuna.trial.Trial, device):
    """Optuna objective.
    Args:
        trial
    Returns:
...
def get_best_trial_with_condition(optuna_study: optuna.study.Study):
...
def tune(gpu_id, storage: str = None):
    ...
    study.optimize(lambda trial: objective(trial, device), n_trials=15)
